...
def lista_uzytkownikow():
    listbox_lista_obietkow.delete(0, END)
    for idx, user in enumerate(users):
        listbox_lista_obietkow.insert(idx, f"{user.imie} {user.nazwisko}")

def pokaz_szczeoly_uzytkownika():
    i=listbox_lista_obietkow.index(ACTIVE)
    print(i)
    imie=users[i].imie
...
def aktualizuj_uzytkownika(i):
    ...
    button_dodaj_uzytkownika.config(text="Dodaj użytkownika", command= dodaj_użytkownika)
...
button_pokaz_szczegoly=Button(ramka_lista_uzytkownikow, text="Pokaż szczegóły", command=pokaz_szczeoly_uzytkownika)
...
button_dodaj_uzytkownika=Button(ramka_formularz, text="Dodaj użytkownika", command= dodaj_użytkownika)
button_dodaj_uzytkownika.grid(row=5, column=0, columnspan=2)


#ramka szczegoly obietków
label_opis_obietku=Label(ramka_szczegoly_obiektu, text="Szczegóły obiektu:")
...
label_opis_obietku.grid(row=0, column=0, sticky=W)
